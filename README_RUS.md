...
3. [Matrix structure in C](#структура-матрицы-на-языке-c)
4. [Matrix operations](#операции-над-матрицами)

![matrix](misc/rus/images/matrixx.png)

## Introduction

Данный проект выполнялся по учебной программе [Школы 21](https://21-school.ru).  
В данном проекте я реализовал аллокацию матрицы в одном участке памяти для эффективности работы.  
![mat   rix_alloc](misc/rus/images/matrix_alloc.png)
Все функции покрыты юнит тестами и задокументированы.

## Техническое заданиче

Реализовать основные действия с матрицами (частично описанные [ниже](#операции-над-матрицами)): create_matrix (создание), remove_matrix (очистка и уничтожение), eq_matrix (сравнение), sum_matrix (сложение), sub_matrix (вычитание), mult_matrix (умножение), mult_number (умножение на число), transpose (транспонирование), determinant (вычисление определителя), calc_complements (вычисление матрицы алгебраических дополнений), inverse_matrix (поиск обратной матрицы).
...
- Матрица должна быть реализована в виде структуры описанной [ниже](#структура-матрицы-на-языке-c)
